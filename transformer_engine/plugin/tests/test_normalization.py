        ...
        for backend_name in self.backends:
            ...
            try:
                output, mean, rsigma = backend.layernorm_fwd(
                    x, weight, bias, self.eps, None, None, torch.float32, 0, False
                )
                self.assert_close(
                    output,
                    ref_output,
        ...
        for backend_name in self.backends:
            ...
            try:
                output, _, rsigma = backend.rmsnorm_fwd(
                    x, weight, self.eps, None, None, torch.float32, 0, False
                )
                self.assert_close(
                    output,
                    ref_output,
        ...
        for backend_name in self.backends:
            ...
            try:
                ...
                grad_x, grad_weight = backend.rmsnorm_bwd(
                    grad_output, x_copy, rsigma.detach(), weight_copy, 0, False, self.eps
                )
